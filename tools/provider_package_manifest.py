...
MANIFEST_RELATIVE_PATH = Path(
    "share/universal-launcher/provider-package-manifest.v1.json"
)
ABI_RELATIVE_PATH = Path(
    "share/universal-launcher/contracts/abi/ulk_c_abi.v1.toml"
)
EXPECTED_PACKAGE_VERSION = "1.9.1"
EXPECTED_C_ABI = {"major": 1, "minor": 9}
EXPECTED_JOURNAL = {"read_versions": [1, 2], "write_version": 2}
EXPECTED_TARGETS = {
...
def installed_targets(prefix: Path) -> list[str]:
    ...
    expression = re.compile(
        r"add_library\((UniversalLauncher::[A-Za-z0-9_]+)\s+"
    )
    for path in prefix.rglob("UniversalLauncherTargets*.cmake"):
        content = path.read_text(encoding="utf-8", errors="replace")
        targets.update(expression.findall(content))
    if "UniversalLauncher::Headers" not in targets:
...
def licence_expression(prefix: Path) -> str:
    record = prefix / "share/licenses/universal-launcher/license.v1.toml"
...
def build_manifest(
    *,
    ...
    headers = select_inventory(artifacts, ("include/ulk/", "include/ulu/"))
    contracts = select_inventory(
        artifacts, ("share/universal-launcher/contracts/",)
    )
    licences = select_inventory(
        artifacts, ("share/licenses/universal-launcher/",)
    )
    ...
    if {"major": abi_major, "minor": abi_minor} != EXPECTED_C_ABI:
        raise ManifestError("installed C ABI must remain exactly 1.9")
    targets = installed_targets(prefix)
    expected_by_linkage = {
        "static": {"UniversalLauncher::Headers", "UniversalLauncher::CoreStatic"},
        "shared": {"UniversalLauncher::Headers", "UniversalLauncher::CoreShared"},
    ...
    return {
        "schema": "ulk.provider_package_manifest.v1",
        "provider": {
...
def verify_manifest(
    ...
    *,
    ...
    expected_journal_read_versions: list[int] | None = None,
    expected_journal_write_version: int | None = None,
    expected_linkage: str | None = None,
) -> dict[str, Any]:
    prefix = prefix.resolve()
    data = load_manifest(prefix)
    if data.get("schema") != "ulk.provider_package_manifest.v1":
        raise ManifestError("provider package manifest schema is unsupported")
    if require_path(data, "provider", "id") != "universal-launcher":
        raise ManifestError("provider identity is not universal-launcher")
    if require_path(data, "provider", "package_version") != EXPECTED_PACKAGE_VERSION:
        raise ManifestError("provider package version is not exact 1.9.1 truth")
    if require_path(data, "provider", "journal") != EXPECTED_JOURNAL:
        raise ManifestError("provider journal identity is not v1/v2 read and v2 write")
    if require_path(data, "provider", "maturity") != "experimental_prerelease":
    ...
    for field, prefixes in (
        ("public_headers", ("include/ulk/", "include/ulu/")),
        ("contracts", ("share/universal-launcher/contracts/",)),
    ):
    ...
    actual_licences = select_inventory(
        actual_artifacts, ("share/licenses/universal-launcher/",)
    )
    ...
    journal = require_path(data, "provider", "journal")
    if (
        expected_journal_read_versions is not None
        and journal.get("read_versions") != expected_journal_read_versions
    ):
        raise ManifestError("journal read-version identity mismatch")
    if (
        expected_journal_write_version is not None
        and journal.get("write_version") != expected_journal_write_version
    ):
        raise ManifestError("journal write-version identity mismatch")
...
def check_repository() -> list[str]:
    ...
    for anchor in (
        "project(universal_launcher VERSION 1.9.1",
    ...
    for path in (ROOT / "README.md", ROOT / "cmake" / "README-SDK.md"):
        content = path.read_text(encoding="utf-8")
        if "UniversalLauncher 1.9.1 EXACT" not in content:
            problems.append(f"{path.relative_to(ROOT)} must use package 1.9.1 EXACT")
    consumer = (ROOT / "tests" / "sdk" / "consumer" / "CMakeLists.txt").read_text(
        encoding="utf-8"
    )
    if 'ULK_FIND_VERSION "1.9.1"' not in consumer:
        problems.append("SDK consumer default must require package 1.9.1")
    session_doc = (ROOT / "docs" / "architecture" / "session_journal.md").read_text(
        encoding="utf-8"
    )
    if "Disk record v2" not in session_doc or "legacy v1 disk records" not in session_doc:
        problems.append("session journal package truth must declare v1 reads and v2 writes")
    session_runtime = (
        ROOT / "runtime" / "launcher" / "kernel" / "ulk_session.c"
    ).read_text(encoding="utf-8")
    for anchor in (
        '"ULK_SESSION_RECORD_V1"',
        '"ULK_SESSION_RECORD_V2|"',
        "disk_version == 1 ? 18u : 19u",
    ):
        if anchor not in session_runtime:
            problems.append(f"session journal format implementation is missing {anchor}")
    abi = (ROOT / "contracts" / "abi" / "ulk_c_abi.v1.toml").read_text(
        encoding="utf-8"
    )
    if "abi_major = 1" not in abi or "abi_minor = 9" not in abi:
        problems.append("provider package manifest requires exact C ABI 1.9")
    return problems
...
def parser() -> argparse.ArgumentParser:
    ...
    verify.add_argument(
        "--expected-journal-read-versions", type=parse_versions
    )
    verify.add_argument("--expected-journal-write-version", type=int)
...
def main() -> int:
    ...
    try:
        ...
        manifest = verify_manifest(
            ...
            expected_journal_read_versions=args.expected_journal_read_versions,
            expected_journal_write_version=args.expected_journal_write_version,
